...
public interface VideoRepository extends JpaRepository<Video, Long> {
    ...
    @Query(
        value = "select distinct video from Video video left join fetch video.user",
        countQuery = "select count(distinct video) from Video video"
    )
    Page<Video> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct video from Video video left join fetch video.user")
    List<Video> findAllWithToOneRelationships();

    @Query("select video from Video video left join fetch video.user where video.id =:id")
